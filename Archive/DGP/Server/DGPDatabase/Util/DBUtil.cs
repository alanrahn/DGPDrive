...
namespace DGPDatabase
{
    public class DBUtil
    {
        ...
        public string ReplicaWrite(SqlCommand sqlCmd, string connStr, string action)
        {
            ...
            using (SqlConnection sqlConn = new SqlConnection(connStr))
            {
                ...
                using (SqlTransaction sqlTran = sqlConn.BeginTransaction())
                {
                    ...
                    if ((action == DBAction.Insert && rowsaff == 1) || (action != DBAction.Insert && rowsaff == 2))
                    {
                        sqlTran.Commit();
        ...
        public string MergeDestRec(SqlCommand sqlCmd, string destTblName, string dupSql, string insertSql, string connStr)
        {
            ...
            string rec_dbname = sqlCmd.Parameters[CmnFields.rec_dbname_prm].Value.ToString();
            string src_state = sqlCmd.Parameters[CmnFields.rec_state_prm].Value.ToString();
            string src_id = sqlCmd.Parameters[CmnFields.src_id_prm].Value.ToString();
            long src_ms = Convert.ToInt64(sqlCmd.Parameters[CmnFields.src_ms_prm].Value);
            ...
            if (destRecs != null && destRecs.Rows.Count > 0)
            {
                ...
                foreach (DataRow dr in destRecs.Rows)
                {
                    ...
                    if (dr[CmnFields.rec_dbname].ToString() == rec_dbname && dr[CmnFields.src_id].ToString() == src_id)
                    {
                        // the record has already been replicated: end the merge process (idempotent)
                        return MethResult.Done;
                    }
                    else
                    {
                        // existing active record with same values (LWW conflict)
                        dest_ms = Convert.ToInt64(dr[CmnFields.row_ms]);
                        destgid = dr[CmnFields.rec_gid].ToString();
                    }
                }
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        sqlCmd.CommandText = insertSql;
                        sqlCmd.Parameters[CmnFields.rec_state].Value = RecState.Edited;
                        action = DBAction.Insert;
                    }
                }
            }
        ...
        public DataTable GetSrcRecs(string srcDbName, string srcTblName, string placeholderID, string maxBatch, string connStr)
        {
            ...
            using (SqlCommand sqlCmd = new SqlCommand())
            {
                ...
                sqlCmd.Parameters.Add(CmnFields.rec_dbname_prm, SqlDbType.VarChar, 50).Value = srcDbName;
