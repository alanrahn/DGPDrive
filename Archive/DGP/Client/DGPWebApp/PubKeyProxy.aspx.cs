...
namespace WebApp
{
    public partial class PubKeyProxy : System.Web.UI.Page
    {
        string keyURL = ConfigurationManager.AppSettings["KeyURL"].ToString();
            ...
            try
            {
                if (ConfigurationManager.AppSettings["LocState"].ToString() == "ONLINE")
                {
                    ...
                    if (HttpContext.Current.Request.ContentType == "text/xml")
                    {
                        ...
                        respMsg = climsgutil.HttpPost(keyURL, requestMsg);
                    }
                    else
                    {
                        string resultMsg = msgutil.CreateXMLResult(MethReturn.MethodError, MethReturn.Default, MethResult.Error, APIData.Text, "Only text/xml request content type is allowed.");
                        respMsg = msgutil.CreateXMLResponse("PubKeyProxy", "", AuthState.MimeType, "Only text/xml request content type is allowed.", "0", resultMsg);
                    }
                }
                else
                {
                    ...
                    respMsg = msgutil.CreateXMLResponse("PubKeyProxy", "", LocState.Offline, "The web service is currently offline.", "0", resultMsg);
                }
            }
            catch (Exception ex)
            {
                RemErrLog.WriteErrToEV("NA", Environment.MachineName, "DGPWebApp", "PubKeyProxy", "Page_Load", "EXCEPTION", ex.Message, ex.StackTrace);
                ...
                respMsg = msgutil.CreateXMLResponse("PubKeyProxy", "", AuthState.Error, "PubKeyProxy.Page_Load exception." + ex.Message, "0", resultMsg);
            }
            finally
            {
